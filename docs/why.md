...
`go-ruby-addressable/addressable` reimplements Ruby's `addressable` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-addressable/addressable` directly, with no Ruby runtime;
